...
public class InstanceOfTreeTransformer
implements StructuredStatementTransformer {
    ...
    private class InstanceTreeRewriter
    extends AbstractExpressionRewriter {
        ...
        public ConditionalExpression rewriteExpression(ConditionalExpression expression, SSAIdentifiers ssaIdentifiers, StatementContainer statementContainer, ExpressionRewriterFlags flags) {
            BooleanOperation bo;
            if (expression instanceof BooleanOperation && (bo = (BooleanOperation)expression).getOp() == BoolOp.AND && bo.getLhs() instanceof BooleanExpression && bo.getRhs() instanceof BooleanOperation) {
                ...
                if (bor.getOp() == BoolOp.AND && bol.getInner() instanceof InstanceOfExpression) {
                    expression = new BooleanOperation(expression.getLoc(), new BooleanOperation(expression.getLoc(), bo.getLhs(), bor.getLhs(), BoolOp.AND), bor.getRhs(), BoolOp.AND);
                }
            }
