...
public class BadCastChainRewriter
extends AbstractExpressionRewriter {
    @Override
    public Expression rewriteExpression(Expression expression, SSAIdentifiers ssaIdentifiers, StatementContainer statementContainer, ExpressionRewriterFlags flags) {
        if ((expression = super.rewriteExpression(expression, ssaIdentifiers, statementContainer, flags)) instanceof CastExpression) {
            ...
            if (type.isComplexType() && childType.isComplexType()) {
                if (!childType.correctCanCastTo(type, null)) {
                    expression = new CastExpression(BytecodeLoc.NONE, expression.getInferredJavaType(), new CastExpression(BytecodeLoc.NONE, new InferredJavaType(TypeConstants.OBJECT, InferredJavaType.Source.UNKNOWN), child, true));
                }
                ...
                expression = new CastExpression(BytecodeLoc.NONE, expression.getInferredJavaType(), child);
            }
        }
        return expression;
    }
